...
func TestNode_Set(t *testing.T) {
	...
	t.Run("succeeds", func(t *testing.T) {
		...
		for i := range 10 {
			...
			assert.EqualValues(t, 1, node.Get(x, y))
			node = node.Set(x, y, 0)
			assert.EqualValues(t, 0, node.Get(x, y))
		}

		// check that not all cells get set
		node = node.Set(1, 1, 1)
		assert.Equal(t, 0, node.Get(2, 2))
	})
}

func TestNode_Get(t *testing.T) {
	node := Empty(1).GrowToFit(55, 233)
	assert.Equal(t, 0, node.Get(55, 233))
	node = node.Set(55, 233, 1)
	assert.Equal(t, 1, node.Get(55, 233))
	treeCorrectness(t, node)
}
